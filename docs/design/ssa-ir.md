...
| float misc | `fcmp.* → u1`, `fext ftrunc itof ftoi`, `bitcast` | `fcmp` ordered; unordered forms, `ftoi` on NaN/range `[OPEN]`. `itof` from `u64` to **`f32`** is refused by name in both emitters — no correctly-rounded one-step sequence exists at the SSE2 baseline and `(float)(double)` rounds twice — so a source that needs it routes `u64 → i64 → f32`: exact while the value is under 2^63, then a single `cvtsi2ss` (`backend_fasmg/emit.inc`'s refusal table; `tests/ir/float_conv.ir`'s header documents the refusal, and Stage 5.3's `examples/pictura/octonaria.exsc` writes the route in the row-index conversion) |
