; tests/unit/bfa_verify_rule8_ok.asm
...
; -----------------------------------------------------------------------------
; verify.inc fixture: rule 8 (numeri): caller and callee declare identical numeri
; Parses the IR text below through parse.inc, runs bfa_verify_func, and
; checks the verdict is exactly BFA_VERIFY_OK (0).
;
...
segment readable writeable
  ...
  outinst: dd 0
  srctext:
  db "functio @r8callee_ok () -> void numeri ad_parem vetita explicita conservata {", 10
  db "b0:", 10
  db "ret", 10
  db "}", 10
  db "functio @r8ok () -> void numeri ad_parem vetita explicita conservata {", 10
  db "b0:", 10
  db "call void @r8callee_ok", 10
  db "ret", 10
  db "}", 10
  .len = $ - srctext
