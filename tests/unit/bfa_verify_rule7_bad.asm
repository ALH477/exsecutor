; tests/unit/bfa_verify_rule7_bad.asm
...
; -----------------------------------------------------------------------------
; verify.inc fixture: rule 7 (red.F): param declares a red.F-typed value -- param is not a permitted producer
; Parses the IR text below through parse.inc, runs bfa_verify_func, and
; checks the verdict is exactly BFA_VERIFY_R7_REDTYPE (7).
;
...
segment readable writeable
  ...
  outinst: dd 0
  srctext:
  db "functio @r7bad () -> void {", 10
  db "b0:", 10
  db "%0 = param red.f32 0", 10
  db "ret", 10
  db "}", 10
  .len = $ - srctext
