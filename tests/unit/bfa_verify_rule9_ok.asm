; tests/unit/bfa_verify_rule9_ok.asm
...
; -----------------------------------------------------------------------------
; verify.inc fixture: rule 9 (no `nop`), well-formed twin. Built through
; ir.inc's own procs directly, NOT parse.inc's text grammar -- there is no
; `nop` mnemonic to spell in text (this file's own FINDING 2), so its bad
; twin (bfa_verify_rule9_bad.asm) pokes `BfaInst.op` to 0 directly after
; building otherwise-valid IR; this twin is the SAME construction with the
; poke omitted, proving the harness itself (not just the absence of the
; poke) is what verdict 0 depends on.
;
