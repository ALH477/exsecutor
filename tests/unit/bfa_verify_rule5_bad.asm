; tests/unit/bfa_verify_rule5_bad.asm
...
; -----------------------------------------------------------------------------
; verify.inc fixture: rule 5: redinit only on one arm of a branch (b1), contrib in the join (b3) it does not dominate
; Parses the IR text below through parse.inc, runs bfa_verify_func, and
; checks the verdict is exactly BFA_VERIFY_R5_REDUCTION (5).
;
...
segment readable executable
  start:
	...
	cmp	eax, BFA_VERIFY_R5_REDUCTION
	jne	.fail2
...
segment readable writeable
  ...
  outinst: dd 0
  srctext:
  db "functio @r5bad (f32) -> f32 {", 10
  db "b0:", 10
  db "%0 = param f32 0", 10
  db "%1 = iconst u64 1", 10
  db "%2 = cmp.eq u64 %1 %1", 10
  db "br %2 b1 b2", 10
  db "b1:", 10
  db "%3 = redinit f32 fadd ordinata 0", 10
  db "%4 = redfin f32 %3", 10
  db "jmp b3", 10
  db "b2:", 10
  db "jmp b3", 10
  db "b3:", 10
