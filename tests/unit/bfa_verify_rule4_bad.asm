; tests/unit/bfa_verify_rule4_bad.asm
...
; -----------------------------------------------------------------------------
; verify.inc fixture: rule 4 (dominance): b2 uses %2, defined only in sibling block b1 -- b1 does not dominate b2
; Parses the IR text below through parse.inc, runs bfa_verify_func, and
; checks the verdict is exactly BFA_VERIFY_R4_DOM (4).
;
...
segment readable writeable
  ...
  outinst: dd 0
  srctext:
  db "functio @r4bad (u64) -> u64 {", 10
  db "b0:", 10
  db "%0 = param u64 0", 10
  db "%1 = cmp.eq u64 %0 %0", 10
  db "br %1 b1 b2", 10
  db "b1:", 10
  db "%2 = addw u64 %0 1", 10
  db "jmp b3", 10
  db "b2:", 10
  db "%3 = addw u64 %2 1", 10
  db "jmp b3", 10
  db "b3:", 10
  db "ret %0", 10
  db "}", 10
